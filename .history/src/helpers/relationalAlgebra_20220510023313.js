...
export function splitQueryIntoBodies(query) {
	...
	Object.entries(bodies['ON']).forEach(([key, value]) => {
        ...
        if (contents[0].trim().split(".").length < 2) {
            let contentLeft = { "table": " ", "atribute": contents[0].trim().split(".")[0] }
            let contentRight = { "table": " ", "atribute": contents[1].trim().split(".")[0] }
        } else {
            let contentLeft = { "table": contents[0].trim().split(".")[0], "atribute": contents[0].trim().split(".")[1] }
            let contentRight = { "table": contents[1].trim().split(".")[0], "atribute": contents[1].trim().split(".")[1] }
        }
