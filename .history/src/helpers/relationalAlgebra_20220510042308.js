...
class TreeOptimizer {
	constructor(query) {
		...
		for (let table of query.JOIN) {
			let aux = new No(this.getTableSelections(table, query.WHERE.expressions), this.getTableAtributes(table, query.SELECT), table);
	...
	getTableAtributes(table, select) {
		...
		for (let i of select.split(",")) {
			...
			this.isAtributeFromTable(table, i)
			if (databaseTable[table].includes(i)) {
				atributes.push(i)
			}
		}
	...
	getTableSelections(table, expressions) {
		let selections = []
	...
	isAtributeFromTable(table, atribute) {
		return databaseTable[table].includes(i)
	}
}
	...
	for (let i of operatorsPriority) {
		for (let j of expressions) {
			if (j.includes(i)) {
				...
				expressionsPriority.push(atribute[0].trim())
				expressions.splice(expressions.indexOf(j), 1)
			}
		}
	}

	bodies['WHERE'] = { "expression": bodies['WHERE'], "expressions": bodies['WHERE'].split("AND").map(i => i.trim()), "atributes": expressionsPriority}
