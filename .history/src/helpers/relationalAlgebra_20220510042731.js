	...
	for (let i of operatorsPriority) {
		for (let j of expressions) {
			if (j.includes(i)) {
				...
				expressionsPriorityAtri.push(atribute[0].trim())
				expressions.splice(expressions.indexOf(j), 1)
			}
		}
	}

	bodies['WHERE'] = { "expression": bodies['WHERE'], "expressions": bodies['WHERE'].split("AND").map(i => i.trim()), "atributes": expressionsPriorityAtri}
