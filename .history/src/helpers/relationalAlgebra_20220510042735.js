...
class TreeOptimizer {
	...
	getTableSelections(table, WHERE) {
		...
		for (let i of WHERE.atributes) {
			if (isAtributeFromTable(table, atribute)) {
				atributes.push(i)
			}
		}
...
export function splitQueryIntoBodies(query) {
	...
	let expressionsPriorityAtributes = []
	...
	for (let i of operatorsPriority) {
		for (let j of expressions) {
			if (j.includes(i)) {
				...
				expressionsPriorityAtributes.push(atribute[0].trim())
				expressions.splice(expressions.indexOf(j), 1)
			}
		}
	}

	bodies['WHERE'] = { "expression": bodies['WHERE'], "expressions": bodies['WHERE'].split("AND").map(i => i.trim()), "atributes": expressionsPriorityAtributes}
